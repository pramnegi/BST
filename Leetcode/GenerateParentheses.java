...
class GenerateParentheses {
	...
	public static void generateParentheses(String str, int left, int right, ArrayList<String> list) {
		if(left == 0 && right == 0) {
			list.add(str);
			return;
		}
		...
		if(left > 0)
			generateParentheses(str + "(", left - 1,right, list);
		if(right > 0)
			generateParentheses(str + ")", left, right - 1, list);
	}
